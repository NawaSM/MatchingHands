package com.example.matchinghandapp;

import androidx.appcompat.app.AppCompatActivity;
import androidx.recyclerview.widget.LinearLayoutManager;
import androidx.recyclerview.widget.RecyclerView;

import android.app.ProgressDialog;
import android.os.Bundle;
import android.os.Handler;
import android.os.Looper;
import android.text.Editable;
import android.text.TextWatcher;
import android.util.Log;
import android.widget.EditText;
import android.widget.ImageView;
import android.widget.Toast;

import com.example.matchinghandapp.utils.GeminiMatchCalculator;
import com.google.firebase.auth.FirebaseAuth;
import com.google.firebase.firestore.DocumentSnapshot;
import com.google.firebase.firestore.FirebaseFirestore;

import java.text.ParseException;
import java.text.SimpleDateFormat;
import java.util.ArrayList;
import java.util.Calendar;
import java.util.Date;
import java.util.HashMap;
import java.util.List;
import java.util.Locale;
import java.util.Map;

public class DiscoverActivity extends AppCompatActivity {

    private RecyclerView recyclerDiscover;
    private DiscoverAdapter adapter;
    private EditText etSearch;
    private ImageView ivSort;
    private ProgressDialog progressDialog;

    private List<OpportunityModel> opportunityList;
    private List<OpportunityModel> filteredList;
    private FirebaseFirestore firestore;
    private int currentSortMode = 0;
    private String userProfile;

    private final Map<String, Boolean> calculatingMatches = new HashMap<>();

    @Override
    protected void onCreate(Bundle savedInstanceState) {
        super.onCreate(savedInstanceState);
        setContentView(R.layout.activity_discover);

        recyclerDiscover = findViewById(R.id.recyclerDiscover);
        recyclerDiscover.setLayoutManager(new LinearLayoutManager(this));

        etSearch = findViewById(R.id.etSearch);
        ivSort = findViewById(R.id.ivSort);

        firestore = FirebaseFirestore.getInstance();

        opportunityList = new ArrayList<>();
        filteredList = new ArrayList<>();

        adapter = new DiscoverAdapter(filteredList, this);
        recyclerDiscover.setAdapter(adapter);
        adapter.preloadBookmarks();

        progressDialog = new ProgressDialog(this);
        progressDialog.setMessage("Loading profile...");
        progressDialog.setCancelable(false);
        progressDialog.show();

        setupSearchBar();
        loadUserProfileFirst(); // Load user profile before loading opportunities.
    }

    // Loads the user's profile data from Firestore first.
    private void loadUserProfileFirst() {
        String uid = FirebaseAuth.getInstance().getCurrentUser() != null
                ? FirebaseAuth.getInstance().getCurrentUser().getUid()
                : null;

        if (uid == null) {
            userProfile = "Anonymous user";
            loadOpportunities();
            return;
        }

        firestore.collection("Users").document(uid).get()
                .addOnSuccessListener(doc -> {
                    if (doc.exists()) {
                        String name = doc.getString("name");
                        String skills = doc.getString("skills");
                        String interests = doc.getString("interests");
                        String experience = doc.getString("experience");
                        String address = doc.getString("address");
                        userProfile = "Name: " + name + "\nSkills: " + skills +
                                "\nInterests: " + interests + "\nExperience: " + experience + "\nLocation: " + address;
                    } else {
                        userProfile = "Anonymous user";
                    }
                    progressDialog.setMessage("Loading opportunities...");
                    loadOpportunities(); // Now load opportunities.
                })
                .addOnFailureListener(e -> {
                    Log.e("DiscoverActivity", "Failed to load profile: " + e.getMessage());
                    userProfile = "Anonymous user";
                    progressDialog.setMessage("Loading opportunities...");
                    loadOpportunities();
                });
    }

    // Loads all opportunities from Firestore.
    private void loadOpportunities() {
        firestore.collection("Opportunities").get()
                .addOnSuccessListener(queryDocumentSnapshots -> {
                    opportunityList.clear();
                    filteredList.clear();

                    if (queryDocumentSnapshots.isEmpty()) {
                        progressDialog.dismiss();
                        Toast.makeText(this, "No opportunities found", Toast.LENGTH_SHORT).show();
                        return;
                    }

                    // Get today's date to filter out past opportunities.
                    Calendar cal = Calendar.getInstance();
                    cal.set(Calendar.HOUR_OF_DAY, 0);
                    cal.set(Calendar.MINUTE, 0);
                    cal.set(Calendar.SECOND, 0);
                    cal.set(Calendar.MILLISECOND, 0);
                    Date today = cal.getTime();

                    SimpleDateFormat sdf = new SimpleDateFormat("dd/MM/yyyy", Locale.getDefault());

                    for (DocumentSnapshot snapshot : queryDocumentSnapshots) {
                        OpportunityModel model = snapshot.toObject(OpportunityModel.class);
                        if (model != null) {
                            // Filter out opportunities with a date in the past.
                            try {
                                Date opportunityDate = sdf.parse(model.getDate());
                                if (opportunityDate != null && opportunityDate.before(today)) {
                                    continue;
                                }
                            } catch (ParseException e) {
                                // If date cannot be parsed, include it by default.
                            }

                            model.setId(snapshot.getId());
                            String organizerId = snapshot.getString("createdBy");
                            if (organizerId != null && !organizerId.trim().isEmpty()) {
                                // Check if the organizer's account is active before showing their opportunities.
                                firestore.collection("Users").document(organizerId).get()
                                        .addOnSuccessListener(userSnap -> {
                                            if (userSnap.exists()) {
                                                Boolean isActive = userSnap.getBoolean("active");
                                                if (isActive != null && !isActive) {
                                                    return; // Skip if organizer is deactivated.
                                                }

                                                String fullName = userSnap.getString("fullname");
                                                model.setAddress(fullName != null ? fullName : "Unknown Organizer");
                                            } else {
                                                model.setAddress("Unknown Organizer");
                                            }
                                            calculateGeminiMatch(model);
                                        })
                                        .addOnFailureListener(e -> {
                                            model.setAddress("Unknown Organizer");
                                            calculateGeminiMatch(model);
                                        });
                            } else {
                                model.setAddress("Unknown Organizer");
                                calculateGeminiMatch(model);
                            }
                        }
                    }
                    progressDialog.dismiss();
                })
                .addOnFailureListener(e -> {
                    progressDialog.dismiss();
                    Toast.makeText(this, "Error: " + e.getMessage(), Toast.LENGTH_SHORT).show();
                });
    }

    // Calculates or fetches the match percentage for an opportunity.
    private void calculateGeminiMatch(OpportunityModel model) {
        String opportunityId = model.getId();
        String currentUserId = FirebaseAuth.getInstance().getCurrentUser().getUid();

        if (calculatingMatches.getOrDefault(opportunityId, false)) {
            return;
        }

        firestore.collection("Opportunities").document(opportunityId).get()
                .addOnSuccessListener(doc -> {
                    if (doc.exists()) {
                        Map<String, Object> userMatches = (Map<String, Object>) doc.get("userMatches");
                        if (userMatches != null && userMatches.containsKey(currentUserId)) {
                            Object matchObj = userMatches.get(currentUserId);
                            if (matchObj instanceof Long) {
                                Long matchValue = (Long) matchObj;
                                // Use the stored match if it's valid (not 0).
                                if (matchValue > 0) {
                                    String storedMatch = matchValue + "% Match";
                                    model.setMatchPercentage(storedMatch);
                                    addOpportunityToLists(model);
                                    return;
                                }
                            }
                        }
                    }
                    // Calculate a new match if one doesn't exist or is 0.
                    calculateAndStoreMatch(model, currentUserId, opportunityId);
                })
                .addOnFailureListener(e -> calculateAndStoreMatch(model, currentUserId, opportunityId));
    }

    // Calculates a new match and stores it in Firestore.
    private void calculateAndStoreMatch(OpportunityModel model, String userId, String opportunityId) {
        calculatingMatches.put(opportunityId, true);
        String opportunityInfo = "Title: " + model.getTitle() +
                "\nDescription: " + model.getDescription() +
                "\nTags: " + model.getTags();

        GeminiMatchCalculator.calculateMatch(userProfile, opportunityInfo)
                .thenAccept(match -> new Handler(Looper.getMainLooper()).post(() -> {
                    if (match > 0 && match <= 100) {
                        String matchStr = match + "% Match";
                        model.setMatchPercentage(matchStr);
                        firestore.collection("Opportunities").document(opportunityId)
                                .update("userMatches." + userId, match)
                                .addOnSuccessListener(aVoid -> Log.d("DiscoverActivity", "Match stored: " + match + "%"))
                                .addOnFailureListener(e -> Log.e("DiscoverActivity", "Failed to store match: " + e.getMessage()));
                    } else {
                        model.setMatchPercentage("N/A");
                    }
                    addOpportunityToLists(model);
                    calculatingMatches.put(opportunityId, false);
                }))
                .exceptionally(e -> {
                    model.setMatchPercentage("N/A");
                    new Handler(Looper.getMainLooper()).post(() -> {
                        addOpportunityToLists(model);
                        calculatingMatches.put(opportunityId, false);
                    });
                    return null;
                });
    }

    // Adds an opportunity to the displayed list.
    private void addOpportunityToLists(OpportunityModel model) {
        opportunityList.add(model);
        if (etSearch.getText().toString().isEmpty()) {
            filteredList.add(model);
        }
        adapter.notifyItemInserted(filteredList.size() - 1);
    }

    // Sets up the search bar and sort functionality.
    private void setupSearchBar() {
        etSearch.addTextChangedListener(new TextWatcher() {
            @Override
            public void beforeTextChanged(CharSequence s, int start, int count, int after) {}

            @Override
            public void onTextChanged(CharSequence s, int start, int before, int count) {
                filterOpportunities(s.toString());
            }

            @Override
            public void afterTextChanged(Editable s) {}
        });

        ivSort.setOnClickListener(v -> {
            currentSortMode = (currentSortMode + 1) % 3;
            sortOpportunities();
            String message = currentSortMode == 0 ? "Default sort" :
                    currentSortMode == 1 ? "Sorted A-Z" : "Sorted by Match %";
            Toast.makeText(this, message, Toast.LENGTH_SHORT).show();
        });
    }

    // Sorts the displayed opportunities.
    private void sortOpportunities() {
        if (currentSortMode == 1) {
            filteredList.sort((o1, o2) -> {
                String title1 = o1.getTitle() != null ? o1.getTitle() : "";
                String title2 = o2.getTitle() != null ? o2.getTitle() : "";
                return title1.compareToIgnoreCase(title2);
            });
        } else if (currentSortMode == 2) {
            filteredList.sort((o1, o2) -> {
                int match1 = extractMatchPercentage(o1.getMatchPercentage());
                int match2 = extractMatchPercentage(o2.getMatchPercentage());
                return Integer.compare(match2, match1);
            });
        }
        adapter.notifyDataSetChanged();
    }

    // Extracts the numeric percentage from the match string.
    private int extractMatchPercentage(String matchStr) {
        if (matchStr == null || matchStr.equals("N/A") || matchStr.equals("Calculating...")) {
            return -1;
        }
        try {
            return Integer.parseInt(matchStr.replaceAll("[^0-9]", ""));
        } catch (Exception e) {
            return -1;
        }
    }

    // Filters the displayed opportunities based on a search query.
    private void filterOpportunities(String query) {
        filteredList.clear();
        if (query.isEmpty()) {
            filteredList.addAll(opportunityList);
        } else {
            String lowerCaseQuery = query.toLowerCase().trim();
            for (OpportunityModel opportunity : opportunityList) {
                boolean matchesTitle = opportunity.getTitle() != null &&
                        opportunity.getTitle().toLowerCase().contains(lowerCaseQuery);
                boolean matchesDescription = opportunity.getDescription() != null &&
                        opportunity.getDescription().toLowerCase().contains(lowerCaseQuery);
                boolean matchesTags = false;
                if (opportunity.getTags() != null) {
                    for (String tag : opportunity.getTags()) {
                        if (tag != null && tag.toLowerCase().contains(lowerCaseQuery)) {
                            matchesTags = true;
                            break;
                        }
                    }
                }
                boolean matchesOrganizer = opportunity.getAddress() != null &&
                        opportunity.getAddress().toLowerCase().contains(lowerCaseQuery);

                if (matchesTitle || matchesDescription || matchesTags || matchesOrganizer) {
                    filteredList.add(opportunity);
                }
            }
        }
        adapter.notifyDataSetChanged();
    }
}
